"""
2026-08-16: use_fast_breakout '만' 썼을 때 - 기존 스퀴즈 state machine(sq/bo 래칫)을
완전히 배제하고, 순수하게 "fast_breakout_lookback개 캔들 전 폭 대비 fast_breakout_mult배
급확장" 조건 하나로만 진입 판정.

트릭: use_fast_breakout는 squeeze_status=="normal"이고 스퀴즈 진입 조건(현재폭 < 평균폭 *
SQUEEZE_ENTER_MULT)이 거짓일 때만 발동하는 보조 경로. SQUEEZE_ENTER_MULT=0으로 두면 그
스퀴즈 진입 조건이 절대 참이 될 수 없어(폭은 항상 >=0) squeeze_status가 영원히 normal에
머무르고, 결과적으로 매 캔들 fast_breakout 조건 하나로만 진입이 결정됨. 코드 수정 없이
파라미터만으로 격리 테스트 가능.

직전 228건 결과(스퀴즈 래칫 + fast_breakout 혼합, PF1.13/MDD61.5%/복리$2889)에서 스퀴즈
래칫 경로를 완전히 빼고 fast_breakout 경로만 남기면 어떻게 되는지 비교.

스코프: XRP만, 15분봉, 365일치, use_hma_direction_only + use_price_alignment_filter,
profit_lock 0.5%/0.85%(기존 베스트와 동일 기준선).
"""
import sys
import os
...
bcl.SQUEEZE_ENTER_MULT = 0.0   # 스퀴즈 상태 진입 자체를 봉쇄 -> 항상 normal -> fast_breakout만 작동
...
print("\n=== FAST BREAKOUT ONLY XRP 1Y TEST COMPLETE ===", flush=True)
