"""
2026-08-17(9차): 직전 테스트(test_squeeze_bo_xrp_1y.py)에서 sq=0.5/bo=1.5(둘 다 낮춤)가
오히려 전체 성과를 악화시킴을 확인. 원인 분석 결과 SQUEEZE_ENTER_MULT는 "폭이 평균의 이
비율 이하로 눌리면 스퀴즈 진입" 조건이라 값을 낮추면 스퀴즈 인정이 더 까다로워짐(진입이
더 느려짐) - 사용자가 원한 "진입 완화" 방향과 반대로 움직인 것. BREAKOUT_MULT는 반대로
낮추면 더 쉽게 발동(완화)이라 두 파라미터가 서로 엇갈려 섞인 결과였음.

사용자 의도("진입이 너무 느린 것 같다" → 더 자주/빠르게 진입하고 싶다)에 맞는 올바른 방향은
SQUEEZE_ENTER_MULT를 올리는 것(0.7→0.8~0.9, 스퀴즈 인정 폭 기준을 넓혀서 더 자주 스퀴즈
상태로 판정되게 함). BREAKOUT_MULT는 라이브 값(1.6) 그대로 유지한 채 SQUEEZE_ENTER_MULT만
격리해서 0.7(라이브)/0.8/0.9 3가지로 비교.
...
VARIANTS = [
    ...
    ("TEST(sq=0.8, bo=1.6)", (0.8, 1.6)),
    ("TEST(sq=0.9, bo=1.6)", (0.9, 1.6)),
]
...
print("\n=== SQUEEZE LOOSEN XRP 1Y TEST COMPLETE ===", flush=True)
