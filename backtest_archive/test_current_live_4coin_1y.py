"""
2026-08-17: "지금 우리 전략"(라이브에 실제 배포된 완전체 조합)을 BTC/ETH/XRP/SOL 4개
코인에 대해 검증. 기존에는 이 완전체 조합(가격정렬필터+fast_breakout+stall_exit 전부
포함)을 XRP 1개에만 테스트했었고(test_stallexit_fastbo_xrp_1y.py), 다른 코인 조합 테스트
(test_fast_breakout_mixed_multi.py)는 stall_exit 추가 전 구버전이라 최신 로직을 반영 못함.
...
print("\n=== CURRENT LIVE 4-COIN (BTC/ETH/XRP/SOL) 1Y TEST COMPLETE ===", flush=True)
