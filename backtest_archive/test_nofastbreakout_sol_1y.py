"""
2026-08-16: 사용자 질문("SOL은 fast_breakout 없을 때 더 좋았던 거 아니야?") 검증용.
이 세션에서 SOL은 fast_breakout-only(64건/PF0.89/-21.88%)와 fast_breakout 믹스
(256건/PF0.95/-29.19%)만 테스트했고, "fast_breakout 아예 없는" sq=0.7/bo=1.6 +
use_hma_direction_only + use_price_alignment_filter 순정 버전은 SOL로 직접 테스트한
적이 없음(XRP만 216건/PF1.10/MDD59.2%로 테스트됨). 실제로 fast_breakout을 빼면
SOL이 더 나아지는지 직접 비교.

설정: sq=0.7/bo=1.6, use_hma_direction_only + use_price_alignment_filter,
profit_lock 0.5%/0.85%, use_fast_breakout=False (나머지 전부 현재 베스트와 동일,
fast_breakout만 뺌). 15분봉, 365일치.
"""
import sys
import os
...
print("\n=== NO FAST BREAKOUT SOL 1Y TEST COMPLETE ===", flush=True)
