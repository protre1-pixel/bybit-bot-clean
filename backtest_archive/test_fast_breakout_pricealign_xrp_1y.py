"""
2026-08-16: use_fast_breakout(2026-08-15에 추가된, SOL 차트 "스퀴즈 선행조건 없이 갑자기
튀는 진짜 브레이크아웃을 놓치고 뒤늦게 추격 진입" 문제 대응용 옵션)를 오늘의 최고 조합
(스퀴즈 state machine sq=0.7/bo=1.6 + use_hma_direction_only + use_price_alignment_filter,
216건/PF1.10/MDD59.2%/복리파산없음) 위에 얹어서 테스트.

기존 state machine을 완전히 대체하는 min_width_breakout/2candle_breakout과 달리, normal
상태에서 스퀴즈 조건이 안 뜬 캔들에서만 보조로 붙는 경로(fast_breakout_lookback=2개 캔들 전
폭 대비 fast_breakout_mult배 이상 급확장 시 즉시 breakout 인정, mult 기본값 None->BREAKOUT_MULT
재사용=1.6). 진입 품질을 희석시키지 않으면서 놓치던 구간만 추가로 잡는지 확인.

스코프: XRP만, 15분봉, 365일치, profit_lock 0.5%/0.85%(기존 베스트와 동일 기준선 - 비교 위해).
"""
import sys
import os
...
print("\n=== FAST BREAKOUT + PRICE ALIGNMENT XRP 1Y TEST COMPLETE ===", flush=True)
