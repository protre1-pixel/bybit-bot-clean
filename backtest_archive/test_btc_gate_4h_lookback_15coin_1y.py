"""
2026-09-05(44차): 43차(test_btc_gate_1h_lookback_15coin_1y.py)에서 1시간 lookback 게이트를
15코인 1년으로 검증한 결과(n=1173, 승률81.4%, PF3.30, MDD12.1%, 수익+$372,484.61)가 나왔는데,
지금 실거래에 배포되어 있는 건 4시간 lookback(41차 기준 4코인·1년만 검증됨)이라 정확히 같은
조건(15코인·1년·같은 캐시 데이터)으로 4시간 lookback도 돌려서 1시간 결과와 직접 비교.

정의: BTC 등락률 = 직전 4시간(16캔들, 15분봉) 종가 대비 현재 종가 등락률(43차와 lookback만
16으로 다름, 나머지 방법론/스코프/LIVE_KWARGS 전부 동일). 43차에서 이미 계산해 둔 "게이트없음"
15코인·1년 기준선(n=5639, 승률64.9%, PF1.23, MDD44.6%, 수익+$279,988.75)을 공통 기준으로 재사용.
...
BTC_GATE_LOOKBACK = 16  # 4시간 = 15분봉 16개 (현재 라이브)
BTC_GATE_THRESHOLD_PCT = 1.0

# 43차 게이트없음 15코인·1년 기준선 (재계산 없이 재사용)
...
print(f"\n[BTC 게이트] 평가구간 {gate_total_count}개 캔들 중 게이트 열림(|4h등락률|>={BTC_GATE_THRESHOLD_PCT}%): "
...
print("|---|---|---|---|---|---|")
print(f"| 게이트없음(43차 기준선) | {BASELINE_NO_GATE['n']} | {BASELINE_NO_GATE['win_rate']:.1f}% | "
      f"{BASELINE_NO_GATE['pf']:.2f} | {BASELINE_NO_GATE['mdd']:.1f}% | ${BASELINE_NO_GATE['profit']:+.2f} |")
print(f"| 게이트있음(4시간,현재라이브) | {overall_gated['n']} | {overall_gated['win_rate']:.1f}% | "
      f"{pfn1} | {overall_gated['mdd']:.1f}% | ${overall_gated['profit']:+.2f} |")
print(f"| 게이트있음(1시간, 43차 기준선) | {BASELINE_1H_GATE['n']} | {BASELINE_1H_GATE['win_rate']:.1f}% | "
      f"{BASELINE_1H_GATE['pf']:.2f} | {BASELINE_1H_GATE['mdd']:.1f}% | ${BASELINE_1H_GATE['profit']:+.2f} |")

print("\n=== BTC GATE 4H LOOKBACK TEST (15 COINS, 1 YEAR) COMPLETE ===", flush=True)
