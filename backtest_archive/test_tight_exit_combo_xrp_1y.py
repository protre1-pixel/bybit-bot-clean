"""
2026-08-16: 가격정렬 필터(use_price_alignment_filter) 적용 후 Trend Follow Stop 172건 진단
결과 - peak가 profit_lock 트리거(0.5%)를 못 찍고 반전된 60건이 "원래 넓은 SL"을 그대로
맞고 나가며 그룹 총손익 -$3,480(25/60 손실)을 만들었고, 반대로 0.5%를 찍어 profit_lock이
걸린 112건은 전승(+$6,914)이었음. 사용자가 제안한 3종 세트를 한번에 적용:

  1) profit_lock_trigger_pct 0.5% → 0.2%로 낮춤 (더 일찍 보호 시작, 기존 파라미터 재사용)
  2) stall_exit: 진입 후 4캔들(1시간)이 지나도록 peak가 0.15%도 못 찍으면 SL을
     entry∓0.8%로 강제로 좁힘 (신규 로직)
  3) entry_sl_cap_pct: 진입 시점 SL 상한을 3.5% → 1.5%로 좁힘 (신규 로직)

진입 타이밍/방향판정은 직전 최고성과 조합(스퀴즈 state machine 그대로 + use_hma_direction_only
+ use_price_alignment_filter) 유지. 이 3종 세트가 그 위에 얹었을 때 효과를 격리해서 봄.
...
PLT, PLR = 0.2, 0.85          # 1) profit_lock 트리거를 낮춤
STALL_CANDLES = 4              # 2) 진입 후 1시간(15분×4) 안에
STALL_MIN_PEAK = 0.15          #    peak가 0.15%도 못 찍으면
STALL_SL_PCT = 0.8             #    SL을 entry∓0.8%로 강제 타이트닝
ENTRY_SL_CAP = 1.5             # 3) 진입 시점 SL 상한을 1.5%로 좁힘
...
print("\n=== TIGHT EXIT COMBO XRP 1Y TEST COMPLETE ===", flush=True)
