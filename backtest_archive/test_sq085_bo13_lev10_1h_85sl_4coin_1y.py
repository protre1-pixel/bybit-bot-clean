"""
2026-08-19(36차): 사용자가 "지금 진입이 너무 느려서 불만"이라며, 진입 완화(SQ0.85/BO1.3)와
동시에 레버리지 10배 + 홀드시간 1시간 + 캐터스트로픽SL 8.5%로 한 번에 바꿔서 배포해달라고
요청. 이 조합(SQ0.85/BO1.3 + 레버10x + 홀드1h + SL8.5%)은 지금까지 한 번도 같이 테스트된 적이
없어서(35차는 SQ0.85/BO1.3을 레버5x/홀드3h/SL16% 위에서만 테스트했음), 배포 전에 먼저
이 정확한 조합으로 백테스트해서 수치를 확인하기로 함.

방법: test_sq085_bo13_lev5_16sl_4coin_1y.py(35차)와 완전히 동일한 하이브리드 패치 구조를
그대로 재사용하고, 딱 세 상수만 교체:
  - MIN_HOLD_CANDLES: 12(3h) -> 4(1h)   [15분봉 x 4 = 1시간]
  - HOLD_CAT_SL_PCT: 16.0 -> 8.5
  - TEST_LEVERAGE: 5 -> 10
SQUEEZE_ENTER_MULT=0.85, BREAKOUT_MULT=1.3는 35차와 동일하게 유지.
나머지(LIVE_KWARGS)는 전부 동일.

비교 대상 1) CURRENT_LIVE - 34차 결과, 현재 실제 배포 근거 수치(SQ0.7/BO1.6, 레버5x,
홀드3h, SL16%), backtest_archive/2026-08-18_leverage5_3h_hold_16sl_deploy.md에 기록:
...
비교 대상 2) SQ085_BO13_LEV5 - 35차 결과, SQ0.85/BO1.3 + 레버5x/홀드3h/SL16%:
BTC n=359 승률63.8% PF2.43 MDD14.6% 비복리+190.7% 캣SL0건
ETH n=367 승률66.8% PF3.57 MDD18.4% 비복리+314.3% 캣SL0건
XRP n=364 승률67.6% PF3.19 MDD13.2% 비복리+293.3% 캣SL0건
SOL n=369 승률68.8% PF3.36 MDD14.7% 비복리+345.3% 캣SL0건
...
INTERVAL = "15"
DAYS = 365
MIN_HOLD_CANDLES = 4  # 15m x 4 = 1시간
HOLD_CAT_SL_PCT = 8.5
TEST_LEVERAGE = 10
...
# 34차(현재 라이브, SQ0.7/BO1.6 + 레버5x/홀드3h/SL16%) - 재실행 안 함
CURRENT_LIVE = {
...
# 35차(SQ0.85/BO1.3 + 레버5x/홀드3h/SL16%) - 재실행 안 함
SQ085_BO13_LEV5 = {
...
for symbol in SYMBOLS:
    ...
    print(f"[{symbol}/SQ0.85_BO1.3_LEV10_1H_85SL] 거래수 {s['n']}건  승률 {s['win_rate']:.1f}%  "
          f"PF {s['pf']:.2f}  MDD {s['mdd']:.1f}%  평균보유 {s['avg_hold']:.1f}h  "
          f"비복리합산 {s['sum_pct']:+.1f}%  캐터스트로픽SL발동 {s['cat_sl_count']}건", flush=True)

print(f"\n{'='*100}\n[종합: 현재라이브(SQ0.7/BO1.6,레버5x,홀드3h,SL16%) vs "
      f"SQ0.85/BO1.3-레버5x-홀드3h-SL16%(35차) vs SQ0.85/BO1.3-레버10x-홀드1h-SL8.5%(신규)]\n{'='*100}")
print("| 심볼 | 버전 | 거래수 | 승률 | PF | MDD | 비복리합산% | 캣SL발동 |")
print("|---|---|---|---|---|---|---|---|")
for symbol in SYMBOLS:
    c = CURRENT_LIVE[symbol]
    m = SQ085_BO13_LEV5[symbol]
    n = results[symbol]
    pfn = f"{n['pf']:.2f}" if n['pf'] != float("inf") else "inf"
    print(f"| {symbol} | 현재라이브(SQ0.7/BO1.6,5x,3h,16%) | {c['n']} | {c['win_rate']:.1f}% | {c['pf']:.2f} | "
          f"{c['mdd']:.1f}% | {c['sum_pct']:+.1f}% | {c['cat_sl_count']}건 |")
    print(f"| {symbol} | SQ0.85/BO1.3,5x,3h,16%(35차) | {m['n']} | {m['win_rate']:.1f}% | {m['pf']:.2f} | "
          f"{m['mdd']:.1f}% | {m['sum_pct']:+.1f}% | {m['cat_sl_count']}건 |")
    print(f"| {symbol} | SQ0.85/BO1.3,10x,1h,8.5%(신규) | {n['n']} | {n['win_rate']:.1f}% | {pfn} | "
          f"{n['mdd']:.1f}% | {n['sum_pct']:+.1f}% | {n['cat_sl_count']}건 |")

print("\n=== SQ0.85/BO1.3 (LEVERAGE 10x, 1H HOLD, 8.5% CAT SL) 4-COIN 1Y TEST COMPLETE ===", flush=True)
