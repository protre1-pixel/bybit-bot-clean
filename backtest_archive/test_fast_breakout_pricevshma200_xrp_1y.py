"""
2026-08-16: 현재 베스트 조합(sq=0.7/bo=1.6 + fast_breakout 믹스 + profit_lock 0.5%/0.85%,
228건/PF1.13/MDD61.5%/복리+188.91%)에서 방향판정 방식만 교체해서 테스트.

기존 베스트는 use_hma_direction_only(HMA200 vs HMA600 정배열/역배열 부호) +
use_price_alignment_filter(가격까지 완전 정배열)로 방향을 정했음. 이번엔 그 대신
use_price_vs_hma200_direction 옵션 사용: 브레이크아웃 캔들 종가가 HMA200(200일선) 위에
있으면 롱, 아래 있으면 숏 - 정배열/역배열(HMA200 vs HMA600) 확인도, 가격정렬필터도 전혀
안 씀. 순수하게 "가격 vs 200선" 단일 기준.

나머지 조건(15분봉, 365일치, sq=0.7/bo=1.6, use_fast_breakout lookback=2/mult=BREAKOUT_MULT
재사용, profit_lock 0.5%/0.85%)은 전부 동일 유지.
...
trades, seed_end = bcl.run_backtest(
    ...
    use_hma_direction_only=False, use_price_alignment_filter=False,
...
print("\n=== FAST BREAKOUT PRICE-VS-HMA200 XRP 1Y TEST COMPLETE ===", flush=True)
