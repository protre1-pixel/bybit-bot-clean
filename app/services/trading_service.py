...
# 2026-09-17: 사용자 요청 - 9/15 실거래 검토 결과 trend_follow 전환 직후 반전한 거래(ATOM
# -2.91%)가 본전방어도 못 받고 넓은 normal단계 SL에 그대로 노출됨을 확인. 0.4→0.3으로 낮춰서
# 본전방어가 좀 더 일찍 걸리게 함(단, 과거 SOL 사례처럼 너무 낮추면 정상 눌림에도 조기청산되는
# 문제가 재발할 수 있어 0.1% 이하로는 내리지 않음 - 실거래 결과 보고 재조정 가능).
# 2026-09-20: 사용자 판단 - "지금 수익을 너무 짧게 끊어 먹는 경향이 있다"며 본전방어 자체를
# 사실상 비활성화(SKIP)하고 profit_lock(트리거/비율)만으로 trend_follow 트레일링을 운용하도록
# 요청. backtest_archive/test_trend_follow_exit_tune_1h_15coin_1y.py 백테스트(15코인·1h·1년,
# BTC 공유게이트)에서는 이 조합이 순수익 -87%(+$22,385→+$2,785), 승률 82.6%→74.2%,
# MDD 21.5%→35.2%로 악화된다는 결과가 나왔음을 확인/보고했으나, 사용자가 결과를 인지한 상태로
# 실거래 재검증을 원해 그대로 적용. 값을 도달 불가능한 큰 수로 두면(peak_profit_pct >= 1e9는
# 항상 False) 아래 본전방어 분기가 사실상 죽은 코드가 되어 "주석처리"와 동일한 실행 결과를
# 만들면서 로직 자체는 남겨둠(실거래 결과 보고 되돌릴 수 있게).
MIN_PROFIT_FOR_BREAKEVEN_PCT = 1e9  # trend_follow 전환 후 이 수익률(%) 넘기 전까진 본전 바닥 SL 자체를 걸지 않음 (SKIP: 도달 불가능한 값으로 사실상 비활성화)
...
# 2026-08-17: ratio를 0.85 → 1.0(무반납)으로 상향. stall_exit/fast_breakout/price_alignment_
# filter/regime_exit이 전부 적용된 현재 라이브 풀 콤보 기준으로 0.9 vs 1.0을 직접 비교
# 재검증(test_profit_lock_ratio09_vs_10_4coin_1y.py, 4코인 15분봉 365일) - 거래수/승률/평균
# 보유시간은 ratio와 무관하게 완전히 동일(같은 캔들에서 청산), PF/MDD/수익률만 4코인 전부
# 예외 없이 1.0이 0.9보다 우세(PF +0.06~0.14, MDD -1~8%p, 비복리합산 +8~13%p). ratio는
# "같은 청산 캔들에서 최고수익 대비 얼마를 반납하고 나가느냐"만 결정하므로 반납을 아예 안
# 하는 1.0이 구조적으로 항상 유리 - 8/16 스윕 때 확인된 "ratio가 높을수록 단조 개선" 패턴과
# 일치. 캐비어트: 15분봉 OHLC는 캔들 내부 저가/고가 도달 "순서"를 모르므로, ratio=1.0(0%
# 쿠션)은 실거래에서 백테스트보다 더 자주 손절가에 걸릴 위험이 있음 - 사용자 승인 하에 채택,
# 실거래에서 이상 반응 시 0.9~0.95 사이로 되돌릴 수 있음.
# 2026-09-17: 사용자 요청 - 9/15 실거래 13건 전부가 "Trend Follow Stop"으로 청산됐는데
# 전부 최고수익 0.25~0.81%대의 자잘한 승리였음(0.5%만 찍어도 즉시 80%를 지키는 SL이 걸려서
# 조금만 눌려도 바로 잘림). trigger를 0.5→0.8로 올려 초반에 좀 더 버티게 하고, ratio를
# 0.8→0.6으로 낮춰 최고점 대비 40%까지 눌려도 견디게 해서 추세가 더 갈 여지를 줌("숨쉴 틈").
# 승률은 다소 낮아질 수 있으나(더 크게 눌린 뒤 확정), 평균 승리 폭 확대가 목적 - 실거래 결과
# 보고 재조정 가능.
# 2026-09-20: 사용자 요청 - 본전방어 SKIP(위 MIN_PROFIT_FOR_BREAKEVEN_PCT 참고)과 함께 트리거
# 0.8→1.0, 비율 0.6→0.5로 변경(최고수익의 절반만 사수, 더 크게 눌려도 버팀). 백테스트상으로는
# 악화 확인됐으나(위 주석 참고) 사용자 승인 하에 실거래 재검증 목적으로 적용.
        ...
        if not state[coin_key]["position"]:
            ...
            try:
                if "last_close_time" in state[coin_key] and state[coin_key]["last_close_time"]:
                    last_close_time = datetime.fromisoformat(state[coin_key]["last_close_time"])
                    elapsed = (datetime.now() - last_close_time).total_seconds()
                    if elapsed < 3600:  # 2026-09-12: 사용자 요청으로 재진입 대기 30분→1시간(3600초)으로 재변경 (전날 재진입 손실 반복 방지)
